...
import numpy as np
import os
os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'garbage_collection_threshold:0.05,max_split_size_mb:32'
...
from pytorch_lightning.utilities.memory import is_cuda_out_of_memory, garbage_collection_cuda
# show cuda devices
...
def load_ply(filename, main_drive="", alternative_drives=[]):
    ...
    # Commenting downsampling if size too big
    
    file_size = os.path.getsize(filename)
    
    ratio = min(int(ceil(float(file_size) / (3*1024*1024))),10)
...
def save_surface_ply(surface_points, normals, colors, score, distance, coeff, n, filename):
    ...
    pcd.points = o3d.utility.Vector3dVector(surface_points.astype(np.float16))
...
def save_single_prediction(surfaces, surfaces_normals, surfaces_colors, scores, names_batch, score_threshold, distance_threshold, n, alpha, slope_alpha):
    save_block_ply(
        surfaces, surfaces_normals, surfaces_colors, scores, 
        names_batch[0], score_threshold, distance_threshold, n, alpha, slope_alpha
    )
...
def post_process_surfaces(surfaces, surfaces_normals, surfaces_colors, scores, score_threshold=0.5, distance_threshold=10.0, n=4, alpha = 1000.0, slope_alpha = 0.1):
    ...
    for i in range(len(surfaces)): # loop over all surface of block
        ...
        if len(surfaces[i]) < 10: # too small surface 50 for the others
            continue
        if scores[i] < score_threshold: # low score
            continue

        if len(surfaces[i]) > 120000: # too many points, likely outlier!
            continue

        # Apply HDBSCAN to find clusters and noise #eps=epsilon
        '''
        db = hdbscan.HDBSCAN(min_samples=10, allow_single_cluster=True).fit(surfaces[i])

        # Get labels assigned by DBSCAN
        cluster_labels = db.labels_

        try:
            largest_cluster_label = max(set(cluster_labels) - {-1}, key=list(cluster_labels).count)
        ...
        # mask sheet
        if len(surfaces[i][points_mask]) >= 0.5*len(surfaces[i]):
        ...
        coeff_list.append(np.array([0,0])) # save coefficients
        distances_list.append(0) # save distances
...
def remove_duplicate_points_normals(points, normals, colors=None, angles=None):
    ...
    Remove duplicate points from a point cloud.
    """
    points = np.round(points, decimals=2).astype(np.float32)
...
def move_to_cpu(prediction):
    """
    Recursively detaches and moves all torch Tensors in the dictionary to CPU.
    Handles nested dictionaries and lists.
    """
    if isinstance(prediction, torch.Tensor):
        # If the prediction is a tensor and on CUDA, detach and move it to CPU
        if prediction.is_cuda:
            return prediction.detach().cpu()
        else:
            return prediction.detach()  # Just detach if it's already on CPU

    elif isinstance(prediction, dict):
        # If it's a dictionary, recursively apply the function to all values
        return {k: move_to_cpu(v) for k, v in prediction.items()}

    elif isinstance(prediction, list):
        # If it's a list, recursively apply the function to all elements
        return [move_to_cpu(item) for item in prediction]

    # For any other type, just return it as is (e.g., non-tensor elements)
    return prediction
...
class MyPredictionWriter(BasePredictionWriter):
    def __init__(self, path="/workspace/scroll3_surface_points", folder="point_cloud_colorized", dest="/media/julian/HDD8TB/scroll3_surface_points", main_drive="", alternative_drives=[], fix_umbilicus=True, umbilicus_points_path="", start=[0, 0, 0], stop=[16, 17, 29], size = [3, 3, 3], umbilicus_distance_threshold=1500, score_threshold=0.5, batch_size=1, gpus=1, num_processes=3):
    ...
    def write_on_batch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule, prediction, batch_indices, batch, batch_idx: int, dataloader_idx: int) -> None:
        ...
        items_pytorch = move_to_cpu(items_pytorch)
        ...
        #wandb.log({"batch_progress": 1}, step=batch_idx)
        #wandb.log({"dl_progress": 1}, step=dataloader_idx)
    ...
    def post_process(self, indxs, res, items_pytorch, points_batch, normals_batch, colors_batch, names_batch, use_multiprocessing=False, distance_threshold=10.0, n=4, alpha=1000.0, slope_alpha=0.1):
        try:
            ...
            if len(points_batch) > 1:
                ...
                if colors_batch[0].shape[0] == 0:
                    colors_batch[0] = np.ones((points_batch[0].shape[0], 3), dtype=np.float16)
                surfaces, surfaces_normals, surfaces_colors, scores = to_surfaces(points_batch[0], normals_batch[0], colors_batch[0], res[0])

            # Save predictions and surfaces
            if len(points_batch) > 1:
                if use_multiprocessing:
                ...
                save_single_prediction(surfaces, surfaces_normals, surfaces_colors, scores, names_batch, self.score_threshold, distance_threshold, n, alpha, slope_alpha)
            ...
            # After processing, clean up GPU memory
            garbage_collection_cuda()

        except RuntimeError as e:
            if is_cuda_out_of_memory(e):
                print("CUDA out of memory error caught. Trying to free memory.")
                garbage_collection_cuda()
                torch.cuda.empty_cache()  # Attempt to release memory and continue
            else:
                raise e  # Re-raise the error if it's not memory related


class PointCloudDataset(Dataset):
    def __init__(self, path="/workspace/scroll3_surface_points", folder="point_cloud_colorized", dest="/media/julian/HDD8TB/scroll3_surface_points", main_drive="", alternative_drives=[], fix_umbilicus=True, umbilicus_points_path="", start=[0, 0, 0], stop=[16, 17, 29], size = [3, 3, 3], umbilicus_distance_threshold=1500, score_threshold=0.5, batch_size=1, gpus=1, num_processes=1, recompute=False):
...
def pointcloud_inference(path, folder, dest, main_drive, alternative_drives, fix_umbilicus, umbilicus_points_path, start, stop, size, umbilicus_distance_threshold, score_threshold, batch_size, gpus, recompute):
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, collate_fn=custom_collate_fn, shuffle=False, num_workers=1, prefetch_factor=1, pin_memory=False)  # Adjust num_workers as per your system

    writer = dataset.get_writer()
    trainer = pl.Trainer(callbacks=[writer], gpus=gpus, precision=16, strategy="ddp") # precision 16 to reduce memory
...
# python3 -m ThaumatoAnakalyptor.pointcloud_to_instances_giorgio --path /scroll1_source/scroll1_surface_points --folder point_cloud_colorized_recto --dest /scroll1_dest/scroll1_surface_points --umbilicus_path /scroll1_dest/scroll1_surface_points/umbilicus.txt --main_drive "" --alternative_ply_drives "" "" --batch_size 1 --gpus 8
...
def compute_overlap_for_pair(args):
    i, patches_list = args
    result_overlapps = []
    #loss = SamplesLoss(loss="sinkhorn", p=2, blur=.1, backend="tensorized").cuda()
    for j in range(i+1, len(patches_list)):
        ...
        if len(overlap_tiles) == 0:
            ...
            #L = loss(torch.from_numpy(patches_list[i]['points']).cuda(), torch.from_numpy(patches_list[j]['points']).cuda()).cpu().item()
            #overlapp_percentage = np.exp(-0/100)
            overlapp_percentage = 1.

        result_overlapps.append((i, j, overlapp_percentage))
...
def surface_overlapp_tiles(patches_list, i, j):
    """
    ...
    loss = SamplesLoss(loss="sinkhorn", p=2, blur=.1, backend="tensorized").cuda()
    L = loss(torch.from_numpy(patch1['points']).cuda(), torch.from_numpy(patch2['points']).cuda()).cpu()

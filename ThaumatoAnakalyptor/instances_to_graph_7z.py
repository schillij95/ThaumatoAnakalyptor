...
from .instances_to_sheets_giorgio import select_points, get_vector_mean, add_overlapp_entries_to_patches_list, assign_points_to_tiles, compute_overlap_for_pair, overlapp_score, fit_sheet, winding_switch_sheet_score_raw_precomputed_surface, find_starting_patch, save_main_sheet, update_main_sheet
...
def score_same_block_patches(patch1, patch2, overlapp_threshold, umbilicus_distance):
    ...
    centroid1 = geometric_median(patch1["points"])
    centroid2 = geometric_median(patch2["points"])
...
def process_same_block(main_block_patches_list, overlapp_threshold, umbilicus_distance):
    ...
    for i in range(len(main_block_patches_list)):
        ...
        for j in range(len(main_block_patches_list)):
            ...
            if score_ > 0.0:
                score_switching_sheets_.append((main_block_patches_list[i]['ids'][0], main_block_patches_list[j]['ids'][0], score_, k_, anchor_angle1, anchor_angle2, geometric_median(main_block_patches_list[i]["points"]), geometric_median(main_block_patches_list[j]["points"])))

            # Add bad edges
            score_bad_edges.append((main_block_patches_list[i]['ids'][0], main_block_patches_list[j]['ids'][0], 1.0, 0.0, anchor_angle1, anchor_angle2, geometric_median(main_block_patches_list[i]["points"]), geometric_median(main_block_patches_list[j]["points"])))
...
def score_other_block_patches(patches_list, i, j, overlapp_threshold):
    ...
    for result in results:
        for i, j, overlapp_percentage in result:
...
def process_block(args):
    ...
    for patch in main_block_patches_list:
        patches_centroids[tuple(patch["ids"][0])] = geometric_median(patch["points"])

    # Extract block's integer ID
    block_id = np.array(extract_ids(file_name))
    ...
    for surrounding_id in surrounding_ids:
        volume_path = os.path.join(path_instances, f"{surrounding_id[0]:06}_{surrounding_id[1]:06}_{surrounding_id[2]:06}")
        ...
        for surrounding_blocks_patches in surrounding_blocks_patches_list:
            ...
            for j, surrounding_block_patch in enumerate(surrounding_blocks_patches):
                ...
                if score_[0] > overlapp_threshold["final_score_min"]:
                    score_sheets_patch.append((main_block_patch['ids'][0], surrounding_block_patch['ids'][0], score_[0], None, score_[1], score_[2], geometric_median(main_block_patch["points"]), geometric_median(surrounding_block_patch["points"])))
...
class ScrollGraph(Graph):
    ...
    def build_graph(self, path_instances, start_point, num_processes=4, prune_unconnected=False):
        blocks_tar_files = glob.glob(os.path.join(path_instances, '*.tar'))
        blocks_7z_files = glob.glob(os.path.join(path_instances, '*.7z'))

        blocks_archives = blocks_tar_files + blocks_7z_files

        # Get the base file names without extensions
        blocks_base_files = [os.path.splitext(f)[0] for f in blocks_archives]
        ...
        print(f"Found {len(blocks_base_files)} blocks.")
        print("Building graph...")

        # Create a pool of worker processes
        with Pool(num_processes) as pool:
            # Map the process_block function to each file
            zipped_args = list(zip(blocks_base_files, [path_instances] * len(blocks_base_files), [self.overlapp_threshold] * len(blocks_base_files), [self.umbilicus_data] * len(blocks_base_files)))
...
def random_walks():
    ...
    overlapp_threshold = {"sample_ratio_score": 1.0, "display": False, "print_scores": True, "picked_scores_similarity": 0.7, "final_score_max": 1.5, "final_score_min": 0.0005, "score_threshold": 0.005, "fit_sheet": False, "cost_threshold": 17, "cost_percentile": 75, "cost_percentile_threshold": 14, 

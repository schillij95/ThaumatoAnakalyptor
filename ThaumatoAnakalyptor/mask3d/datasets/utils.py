...
class VoxelizeCollate:
    def __init__(
        ...
        self.non_labeled_class = non_labeled_class
    ...
    def __call__(self, batch):
        ...
        if ("train" in self.mode) and self.very_small_crops:
            batch = make_crops(batch)
        data, target, file_names = voxelize(
            batch,
            self.ignore_label,
            self.voxel_size,
            self.probing,
        ...
        for i, sample in enumerate(batch):
            target[i]["original_labels"] = data.original_labels[i]
            if len(sample[2]) > 0:
                # Add non labeled class removal masking
                if self.non_labeled_class is not None:
                    target[i]['unlabeled_point'] = self.non_labeled_class
        
        return data, target, file_names
...
def voxelize(
    ...
    for sample in batch:
        ...
        if len(sample[2]) > 0:
            ...
            labels_unique.append(sample_labels)
    ...
    if "labels" in input_dict:
        ...
        for u, t in enumerate(target):
            t["labels_unique"] = labels_unique[u]
    else:
        target = []
        target_full = []
        coordinates = []

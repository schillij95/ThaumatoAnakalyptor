...
def init_worker(graph, graph_shape, factor_0, factor_not_0):
...
def build_graph_from_individual(individual, graph_raw, return_valid_mask=False):
...
def evalItemsPatchSelection(individual):
    ...
    valid_mask, valid_edges_count = build_graph_from_individual_patch(individual, graph_raw, return_valid_mask=True)
    return (valid_edges_count,)
...
def evalItems(individual):
    ...
    graph_raw = np.frombuffer(var_dict['graph'], dtype=np.int32).reshape(var_dict['graph_shape'])
    valid_edges_count = build_graph_from_individual(individual, graph_raw)
    return (valid_edges_count,)
...
def solve_(graph_edges, problem='k_assignment'):
    if problem == 'k_assignment':
...
def solve_k_assignment_(graph_edges):
    ...
    var_dict['factor_0'] = factor_0
    var_dict['factor_not_0'] = factor_not_0
    valid_mask, valid_edges_count = build_graph_from_individual(best_individual, graph_edges, return_valid_mask=True)
    return valid_mask, valid_edges_count

def solve(graph_edges, problem='k_assignment'):
    # K factors
    if problem == 'k_assignment':
        factor_0, factor_not_0 = calculate_fitness_k_factors(graph_edges)
    else:
    ...
    # child process.)
    with Pool(initializer=init_worker, initargs=(graph_edges_raw_array, graph_shape, factor_0, factor_not_0)) as pool:
        ...
        # get the valid mask
        if problem == 'k_assignment':
            valid_mask, valid_edges_count = build_graph_from_individual(best_ind, graph_edges, return_valid_mask=True)
        else:
